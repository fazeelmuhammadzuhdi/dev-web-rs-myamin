...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        ...
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="<?= site_url('filedownloads/create') ?>" class="btn btn-primary">Add File Download</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        $('#tableFile').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '<?= site_url('filedownloads/getData') ?>',
            order: [],
            columnDefs: [{
                targets: 0,
                orderable: false,
    ...
    function hapus(id, judul) {
        Swal.fire({
            title: 'Hapus Data File Download',
            text: 'Yakin Menghapus File Download Dengan Id : ' + id + ' Ini?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'delete',
                    url: '<?= site_url('filedownloads/delete/') ?>' + id,
                    dataType: 'json',
    ...
    function edit(id) {
        window.location = '<?= site_url('filedownloads/edit/') ?>' + id;
    }
</script>
