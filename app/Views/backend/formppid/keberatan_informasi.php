    ...
    <div class="row align-items-center">
        ...
        <div class="col-md-4 col-lg-4 d-flex justify-content-end">
            <div class="widgetbar mr-2">
                <button onclick="cetakLaporanKeberatanInformasiPPID()" class="btn btn-danger">Cetak</button>
            </div>
            <div class="widgetbar">
                <a href="<?= site_url('keberataninformasi/create') ?>" class="btn btn-primary">Add Keberatan Informasi</a>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#tableBanner').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '<?= site_url('keberataninformasi/getData') ?>',
            order: [],
            columnDefs: [{
                targets: 0,
                orderable: false,
    ...
    function hapus(id, judul) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'delete',
                    url: '<?= site_url('keberataninformasi/delete/') ?>' + id,
                    dataType: 'json',
    ...
    function edit(id) {
        window.location = '<?= site_url('keberataninformasi/edit/') ?>' + id;
    }

    function cetakLaporanKeberatanInformasiPPID() {
        window.open("/laporan/keberatan-informasi-ppid", "_blank");
    }
</script>
