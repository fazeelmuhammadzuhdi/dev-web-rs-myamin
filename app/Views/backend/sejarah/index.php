...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        ...
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="<?= site_url('sejarahs/create') ?>" class="btn btn-primary">Add Sejarah</a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        $('#tableBanner').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '<?= site_url('sejarahs/getData') ?>',
            order: [],
            columnDefs: [{
                targets: 0,
                orderable: false,
    ...
    function hapus(id, judul) {
        Swal.fire({
            title: 'Hapus Data Sejarah',
            text: 'Yakin Menghapus Sejarah Dengan Tahun : ' + judul + ' Ini?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'delete',
                    url: '<?= site_url('sejarahs/delete/') ?>' + id,
                    dataType: 'json',
    ...
    function edit(id) {
        window.location = '<?= site_url('sejarahs/edit/') ?>' + id;
    }
</script>
