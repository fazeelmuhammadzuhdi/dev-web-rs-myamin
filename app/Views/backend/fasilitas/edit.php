    ...
    <div class="row">
        ...
        <div class="col-lg-12">
            <div class="card m-b-30">
                ...
                <div class="card-body">
                    <form action="<?= base_url('fasilitasumum/update'); ?>" method="POST" enctype="multipart/form-data">
                        <?= csrf_field(); ?>

                        <input type="hidden" name="_method" value="PUT">
                        <input type="hidden" name="idfasilitas" value="<?= $fasilitas['idfasilitas'] ?>">

                        <div class="form-group">
                            <label for="inputAddress">Nama</label>
                            <input type="text" class="form-control <?= (session()->getFlashdata('error_nama')) ? 'is-invalid' : '' ?>" name="nama" autofocus value="<?= old('nama', $fasilitas['nama']) ?>">
                        ...
                        <div class="form-group">
                            ...
                            <textarea class="editor form-control <?= (session()->getFlashdata('error_keterangan')) ? 'is-invalid' : '' ?>"  name="keterangan" placeholder="Inputkan keterangan Referensi"><?= old('keterangan', strip_tags($fasilitas['keterangan'])) ?></textarea>
                        ...
                        <div class="form-group">
                            <div class="form-check form-check-inline">
                                <input class="form-check-input <?= (session()->getFlashdata('error_status')) ? 'is-invalid' : '' ?>" required type="radio" name="status" id="inlineRadio1" value="Y" <?= ($fasilitas['status'] == 'Y') ? 'checked' : '' ?>>
                                <label class="form-check-label" for="inlineRadio1">Y</label>
                            </div>
                            <div class="form-check form-check-inline">
                                <input class="form-check-input <?= (session()->getFlashdata('error_status')) ? 'is-invalid' : '' ?>" required type="radio" name="status" id="inlineRadio2" value="T" <?= ($fasilitas['status'] == 'T') ? 'checked' : '' ?>>
