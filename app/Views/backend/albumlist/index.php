...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        ...
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <a href="<?= site_url('albumlists/create') ?>" class="btn btn-primary">Add <?= $title ?></a>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    $(document).ready(function() {
        $('#tableBanner').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '<?= site_url('albumlists/getData') ?>',
            order: [],
            columnDefs: [{
                targets: 0,
                orderable: false,
    ...
    function hapus(id, judul) {
        Swal.fire({
            title: 'Hapus Data Album List',
            text: 'Yakin Menghapus Album List Dengan Judul : ' + judul + ' Ini?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'delete',
                    url: '<?= site_url('albumlists/delete/') ?>' + id,
                    dataType: 'json',
    ...
    function edit(id) {
        window.location = '<?= site_url('albumlists/edit/') ?>' + id;
    }

    function detail(id) {
        window.location = '<?= site_url('albumlists/detail/') ?>' + id;
    }
</script>
