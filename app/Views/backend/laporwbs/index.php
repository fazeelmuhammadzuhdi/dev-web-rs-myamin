...
<div class="breadcrumbbar">
    <div class="row align-items-center">
        ...
        <div class="col-md-4 col-lg-4">
            <div class="widgetbar">
                <button onclick="cetakLaporanWbs()" class="btn btn-danger">Cetak</button>
            </div>
        </div>
...
<script>
    ...
    $(document).ready(function() {
        $('#tableKategori').DataTable({
            ...
            processing: true,
            serverSide: true,
            ajax: '<?= site_url('laporwbs/getData') ?>',
            order: [],
            columnDefs: [{
                targets: 0,
                orderable: false,
    ...
    function hapus(id, nama) {
        Swal.fire({
            title: 'Hapus Data WhistleBlowing System',
            text: 'Yakin Menghapus Wbs Dengan Nama : ' + nama + ' Ini?',
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                $.ajax({
                    type: 'delete',
                    url: '<?= site_url('laporwbs/delete/') ?>' + id,
                    dataType: 'json',
    ...
    function cetakLaporanWbs() {
        window.open("/laporan/whistleblowing", "_blank");
    }
</script>
